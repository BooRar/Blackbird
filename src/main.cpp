...
//};
	...
	{
	 std::string date;
 	 std::string exhan1Name;
 	 double echan1Ask;
	 double echan1Bid;
	// std::string exhan2Name;
	// std::string echan2Ask;
	// std::string echan2Bid;
	// std::string exhan3Name;
	// std::string echan3Ask;
	// std::string echan3Bid;
	// std::string exhan4Name;
	// std::string echan4Ask;
	// std::string echan4Bid;
...
//output operator 
std::ostream& operator<<(std::ostream& stream, const vertec& vertec)
{
	stream << vertec.x << "," << vertec.y << "," << vertec.z<<","<<vertec.foo;
	return stream;
}
//overide the second output operator
std::ostream& operator<<(std::ostream& stream2, const TradeData& trade)
{
	stream2 << trade.date << "," << trade.exhan1Name << "," << trade.echan1Ask << "," << trade.echan1Bid;
						 // << "," << trade.exhan2Name << "," << trade.echan2Ask << "," << trade.echan2Bid
						 // << "," << trade.exhan3Name << "," << trade.echan3Ask << "," << trade.echan3Bid
						 // << "," << trade.exhan4Name << "," << trade.echan4Ask << "," << trade.echan4Bid;
	return stream2;
} 


// 'main' function. // Blackbird doesn't require any arguments for now.
int main(int argc, char** argv)
{
		...
		//pipe is for transferring the data to the socket....
		mypipes pip;

		//pip.writePipe("fo this is it");
 
std::cout <<pip.fetchMe()<<std::endl;
printf("sum = %d/n",addnumbers(6,6));

//lets check the nested structure 
//pip::Appointment roo;

std::vector<TradeData> trade;
 //trade.push_back({"2012", "Bitfinex", "9451" ,"5694" ,"Bitstamp", "9451" ,"5694" ,"Kraken", "9451" ,"5694" ,"Cexio", "9451" ,"5694" });
 //trade.push_back({"2013", "Bitfinex", "9451" ,"5694" ,"Bitstamp", "9451" ,"5694" ,"Kraken", "9451" ,"5694" ,"Cexio", "9451" ,"5694" });


std::vector<vertec> vertices;
	vertices.push_back({ 1,2,3,"ep is the function ofg choice " });
	vertices.push_back({ 4,5,6,"ed is the other function you can use " });
	...
	std::cout << ss.str() << std::endl;
        ...
        std::cout<<R"(
                ============================================
                 ___ _      _   ___ _  __  ___ ___ ___ ___
                | _ ) |    /_\ / __| |/ / | _ )_ _| _ \   \
                | _ \ |__ / _ \ (__| ' <  | _ \| ||   / |) |
                |___/____/_/ \_\___|_|\_\ |___/___|_|_\___/
                ============================================
        ...
        std :: cout  << "===> " << RED <<" Jono's Edit FILE FOR EXAMPLE \n\n "<< NORMAL  << std ::endl ;
        ...
        if (params.bitfinexEnable && (params.bitfinexApi.empty() == false || params.isDemoMode))
        {
                ...
                //std::cout<<RED<<getQuote[index]<<std::endl;
        ...
        std::transform(getAvail, getAvail + numExch,
			...
			std::cout << tmp.leg1 << std::endl;
			std::cout << tmp.leg << std::endl;
                        return tmp;
                } );

/*	if (true)
	{
	//	std::cout << getAvail << std::endl;
		std::transform(getAvail, getAvail + numExch,
			begin(balance),
			[&params](decltype(*getAvail) apply)
		{
	...
	*/
                ...
                for (int i = 0; i < numExch; ++i)
                {
                        ...
                        if (balance[i].leg1 > 0.0050 && !inMarket)
                ...
                while (stillRunning)
                {
						...
						//Jono Debugging Purposes 
					//	for (int t = 0; t < trade.size(); t++)
					//	{
						//	if (t < trade.size()) {
								//std::cout << trade[t] << std::endl;
						//		std::string askStr = std::to_string(trade[t].echan1Ask);
						//		std::string bidStr = std::to_string(trade[t].echan1Bid);

								//std::string foo = trade[t];
							//	pip.writePipeFork(trade[t].date + " " + trade[t].exhan1Name + " " + askStr + " " + bidStr);
								//pip.writePipeFork("--------------------------------------------------------------------");

								//pip.writePipeFork(date + " " + name + " " + askStr + " " + bidStr);
								//trade.clear();
						//	}
						//	else {
						//		trade.clear();
						...
						// Jono Post this vector to the fifo pipe

						
						//Jono Clear this vector
                        ...
                        for (int i = 0; i < numExch; ++i)
                        {
                                ...
                                //jonoEdit
                                //std::cout<<RED<<"quote--"<< quote <<"bid--"<< bid <<"ask--"<< ask <<std::endl;
                                std::cout << NORMAL <<"\n | "<< params.exchName[i] <<" \t | ";
                                //printf(quote);
                                std::cout <<" BID |"<< RED << round(bid) << NORMAL <<"\t| "  ;
                                //printf("%.7lf\n",bid);
                                std::cout <<" ASK |" << GREEN << round(ask) <<NORMAL<<"\t|";
                                //printf("%.7lf\n",ask);
                                //std::cout<<CYAN<<currTime;

                                // Saves the bid/ask into the SQLite database
                                addBidAskToDb(dbTableName[i], printDateTimeDb(currTime), bid, ask, params);
								
								std::string  date = printDateTime(currTime);
								std::string  name = params.exchName[i];
								std::string  askStr = std::to_string(ask);
								std::string  bidStr = std::to_string(bid);
								std::string  iterator = std::to_string(i);
								
								//bid and ask to two decimal places 
								double bidvalue, askvalue;

								//bidvalue = std::ceil(bid * 100.0) / 100.0;
								askvalue = std::floor((ask * 100) + .5) / 100;
								bidvalue = std::floor((bid * 100) + .5) / 100;
								...
								//	std::cout << "I is 0" << std::endl;
									...
									if (-1 != open("/tmp/page0", O_CREAT, 0777)) {
										key = ftok("/tmp/page0", 0);
									}
									else {
										perror("open");
										exit(1);
									...
									boost::property_tree::ptree pt;
									pt.put("Date", date);
									pt.put("Exchng.name", name);
									pt.put("Exchng.bid", bidvalue);
									pt.put("Exchng.ask", askvalue);
									...
									//int n = ss.str().length();// dont need to use the length here because it leave old chars in memory

									//copy into shared memory                                                                                                                                                                                 ", 156);
									memcpy(str, ss.str().c_str(), 170);
								...
								//	std::cout << "I is 1" << std::endl;
									...
									if (-1 != open("/tmp/page1", O_CREAT, 0777)) {
										key1 = ftok("/tmp/page1", 0);
									}
									else {
										perror("open");
										exit(1);
									...
									// ftok to generate unique key 
									//key_t key1 = ftok("shmfile1", 60);

									// shmget returns an identifier in shmid 
									int shmid1 = shmget(key1, 170, 0666 | IPC_CREAT);
									...
									pt1.put("Date", date);
									pt1.put("Exchng.name", name);
									pt1.put("Exchng.bid", bidvalue);
									pt1.put("Exchng.ask", askvalue);

									std::stringstream ss1;
									boost::property_tree::json_parser::write_json(ss1, pt1);
								...
								//	std::cout << "I is 2" << std::endl;
									...
									if (-1 != open("/tmp/page2", O_CREAT, 0777)) {
										key2 = ftok("/tmp/page2", 0);
									}
									else {
										perror("open");
										exit(1);
									...
									// ftok to generate unique key 
									//key_t key2 = ftok("shmfile2", 61);

									// shmget returns an identifier in shmid 
									int shmid2 = shmget(key2, 170, 0666 | IPC_CREAT);
									...
									pt2.put("Date", date);
									pt2.put("Exchng.name", name);
									pt2.put("Exchng.bid", bidvalue);
									pt2.put("Exchng.ask", askvalue);
								...
								//	std::cout << "I is 3" << std::endl;
									...
									if (-1 != open("/tmp/page3", O_CREAT, 0777)) {
										key3 = ftok("/tmp/page3", 0);
									}
									else {
										perror("open");
										exit(1);
									...
									// ftok to generate unique key 
									//key_t key3 = ftok("shmfile3", 62);

									// shmget returns an identifier in shmid 
									int shmid3 = shmget(key3, 170, 0666 | IPC_CREAT);
									...
									pt3.put("Date", date);
									pt3.put("Exchng.name", name);
									pt3.put("Exchng.bid", bidvalue);
									pt3.put("Exchng.ask", askvalue);

									std::stringstream ss3;
									boost::property_tree::json_parser::write_json(ss3, pt3);
                        ...
                        if (!inMarket)
                        {
                                //std::cout <<"INFO: Not IN Market"<< std::endl;
                                for (int i = 0; i < numExch; ++i)
                                {
                                        for (int j = 0; j < numExch; ++j)
                                        {
                        ...
                        {
                                //std::cout <<"INFO:  IN Market"<< std::endl;
